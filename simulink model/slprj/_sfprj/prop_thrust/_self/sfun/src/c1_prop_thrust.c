...
#include <stddef.h>
#include "blas.h"
#include "prop_thrust_sfun.h"
#include "c1_prop_thrust.h"
#include "mwmathutil.h"
#define CHARTINSTANCE_CHARTNUMBER      (chartInstance->chartNumber)
#define CHARTINSTANCE_INSTANCENUMBER   (chartInstance->instanceNumber)
#include "prop_thrust_sfun_debug_macros.h"
...
static void initialize_c1_prop_thrust(SFc1_prop_thrustInstanceStruct
  *chartInstance);
static void initialize_params_c1_prop_thrust(SFc1_prop_thrustInstanceStruct
  *chartInstance);
static void enable_c1_prop_thrust(SFc1_prop_thrustInstanceStruct *chartInstance);
static void disable_c1_prop_thrust(SFc1_prop_thrustInstanceStruct *chartInstance);
static void c1_update_debugger_state_c1_prop_thrust
  (SFc1_prop_thrustInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c1_prop_thrust
  (SFc1_prop_thrustInstanceStruct *chartInstance);
static void set_sim_state_c1_prop_thrust(SFc1_prop_thrustInstanceStruct
  *chartInstance, const mxArray *c1_st);
static void finalize_c1_prop_thrust(SFc1_prop_thrustInstanceStruct
  *chartInstance);
static void sf_c1_prop_thrust(SFc1_prop_thrustInstanceStruct *chartInstance);
static void initSimStructsc1_prop_thrust(SFc1_prop_thrustInstanceStruct
  *chartInstance);
static void init_script_number_translation(uint32_T c1_machineNumber, uint32_T
  c1_chartNumber);
static const mxArray *c1_sf_marshallOut(void *chartInstanceVoid, void *c1_inData);
static real_T c1_emlrt_marshallIn(SFc1_prop_thrustInstanceStruct *chartInstance,
  const mxArray *c1_thrust, const char_T *c1_identifier);
static real_T c1_b_emlrt_marshallIn(SFc1_prop_thrustInstanceStruct
...
static real_T c1_interp1(SFc1_prop_thrustInstanceStruct *chartInstance, real_T
  c1_varargin_1[16], real_T c1_varargin_2[16], real_T c1_varargin_3);
static void c1_eml_error(SFc1_prop_thrustInstanceStruct *chartInstance);
static void c1_b_eml_error(SFc1_prop_thrustInstanceStruct *chartInstance);
static const mxArray *c1_c_sf_marshallOut(void *chartInstanceVoid, void
  *c1_inData);
static int32_T c1_c_emlrt_marshallIn(SFc1_prop_thrustInstanceStruct
  *chartInstance, const mxArray *c1_u, const emlrtMsgIdentifier *c1_parentId);
static void c1_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c1_mxArrayInData, const char_T *c1_varName, void *c1_outData);
static uint8_T c1_d_emlrt_marshallIn(SFc1_prop_thrustInstanceStruct
  *chartInstance, const mxArray *c1_b_is_active_c1_prop_thrust, const char_T
  *c1_identifier);
static uint8_T c1_e_emlrt_marshallIn(SFc1_prop_thrustInstanceStruct
  *chartInstance, const mxArray *c1_u, const emlrtMsgIdentifier *c1_parentId);
static void init_dsm_address_info(SFc1_prop_thrustInstanceStruct *chartInstance);
  ...
  *chartInstance)
{
  ...
  chartInstance->c1_is_active_c1_prop_thrust = 0U;
}
  ...
  (SFc1_prop_thrustInstanceStruct *chartInstance)
{
  ...
  c1_b_hoistedGlobal = chartInstance->c1_is_active_c1_prop_thrust;
  ...
  *chartInstance, const mxArray *c1_st)
{
  ...
  *c1_thrust = c1_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c1_u,
    0)), "thrust");
  chartInstance->c1_is_active_c1_prop_thrust = c1_d_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c1_u, 1)),
     "is_active_c1_prop_thrust");
  sf_mex_destroy(&c1_u);
  c1_update_debugger_state_c1_prop_thrust(chartInstance);
...
static void sf_c1_prop_thrust(SFc1_prop_thrustInstanceStruct *chartInstance)
{
  ...
  *c1_b_thrust = c1_thrust;
  ...
  _SFD_SYMBOL_SCOPE_POP();
  _SFD_CHECK_FOR_STATE_INCONSISTENCY(_prop_thrustMachineNumber_,
...
static const mxArray *c1_sf_marshallOut(void *chartInstanceVoid, void *c1_inData)
{
  ...
  const mxArray *c1_y = NULL;
  SFc1_prop_thrustInstanceStruct *chartInstance;
  chartInstance = (SFc1_prop_thrustInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c1_prop_thrust_get_eml_resolved_functions_info(void)
{
  ...
  *c1_identifier)
{
  ...
  c1_y = c1_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c1_b_is_active_c1_prop_thrust), &c1_thisId);
  sf_mex_destroy(&c1_b_is_active_c1_prop_thrust);
  return c1_y;
}
...
void sf_c1_prop_thrust_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c1_prop_thrust_get_autoinheritance_info(void)
{
...
mxArray *sf_c1_prop_thrust_third_party_uses_info(void)
{
  mxArray * mxcell3p = mxCreateCellMatrix(1,0);
  return(mxcell3p);
}

mxArray *sf_c1_prop_thrust_updateBuildInfo_args_info(void)
{
  mxArray *mxBIArgs = mxCreateCellMatrix(1,0);
  return mxBIArgs;
}

static const mxArray *sf_get_sim_state_info_c1_prop_thrust(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"thrust\",},{M[8],M[0],T\"is_active_c1_prop_thrust\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c1_prop_thrust_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc1_prop_thrustInstanceStruct *chartInstance;
    chartInstance = (SFc1_prop_thrustInstanceStruct *) ((ChartInfoStruct *)
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        if (chartAlreadyPresent==0) {
          ...
          init_script_number_translation(_prop_thrustMachineNumber_,
            chartInstance->chartNumber);
          sf_debug_set_chart_disable_implicit_casting
            (sfGlobalDebugInstanceStruct,_prop_thrustMachineNumber_,
      ...
      sf_debug_reset_current_state_configuration(sfGlobalDebugInstanceStruct,
        _prop_thrustMachineNumber_,chartInstance->chartNumber,
        chartInstance->instanceNumber);
    }
  }
}
...
static void sf_opaque_initialize_c1_prop_thrust(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc1_prop_thrustInstanceStruct*) chartInstanceVar)
    ->S,0);
  initialize_params_c1_prop_thrust((SFc1_prop_thrustInstanceStruct*)
    chartInstanceVar);
  initialize_c1_prop_thrust((SFc1_prop_thrustInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_enable_c1_prop_thrust(void *chartInstanceVar)
{
  enable_c1_prop_thrust((SFc1_prop_thrustInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_disable_c1_prop_thrust(void *chartInstanceVar)
{
  disable_c1_prop_thrust((SFc1_prop_thrustInstanceStruct*) chartInstanceVar);
}

static void sf_opaque_gateway_c1_prop_thrust(void *chartInstanceVar)
{
  sf_c1_prop_thrust((SFc1_prop_thrustInstanceStruct*) chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c1_prop_thrust(SimStruct* S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c1_prop_thrust
    ((SFc1_prop_thrustInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c1_prop_thrust();/* state var info */
...
extern void sf_internal_set_sim_state_c1_prop_thrust(SimStruct* S, const mxArray
  *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c1_prop_thrust((SFc1_prop_thrustInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c1_prop_thrust(SimStruct* S)
{
  return sf_internal_get_sim_state_c1_prop_thrust(S);
}

static void sf_opaque_set_sim_state_c1_prop_thrust(SimStruct* S, const mxArray
  *st)
{
  sf_internal_set_sim_state_c1_prop_thrust(S, st);
}

static void sf_opaque_terminate_c1_prop_thrust(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc1_prop_thrustInstanceStruct*) chartInstanceVar)->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
      unload_prop_thrust_optimization_info();
    }

    finalize_c1_prop_thrust((SFc1_prop_thrustInstanceStruct*) chartInstanceVar);
...
{
  initSimStructsc1_prop_thrust((SFc1_prop_thrustInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c1_prop_thrust(SimStruct *S)
{
  int i;
...
static void mdlSetWorkWidths_c1_prop_thrust(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
    mxArray *infoStruct = load_prop_thrust_optimization_info();
...
static void mdlRTW_c1_prop_thrust(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c1_prop_thrust(SimStruct *S)
{
  SFc1_prop_thrustInstanceStruct *chartInstance;
  chartInstance = (SFc1_prop_thrustInstanceStruct *)utMalloc(sizeof
    (SFc1_prop_thrustInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc1_prop_thrustInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway = sf_opaque_gateway_c1_prop_thrust;
  chartInstance->chartInfo.initializeChart = sf_opaque_initialize_c1_prop_thrust;
  chartInstance->chartInfo.terminateChart = sf_opaque_terminate_c1_prop_thrust;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c1_prop_thrust;
  chartInstance->chartInfo.disableChart = sf_opaque_disable_c1_prop_thrust;
  chartInstance->chartInfo.getSimState = sf_opaque_get_sim_state_c1_prop_thrust;
  chartInstance->chartInfo.setSimState = sf_opaque_set_sim_state_c1_prop_thrust;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c1_prop_thrust;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c1_prop_thrust;
  chartInstance->chartInfo.mdlStart = mdlStart_c1_prop_thrust;
  chartInstance->chartInfo.mdlSetWorkWidths = mdlSetWorkWidths_c1_prop_thrust;
...
void c1_prop_thrust_method_dispatcher(SimStruct *S, int_T method, void *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c1_prop_thrust(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c1_prop_thrust(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c1_prop_thrust(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c1_prop_thrust_method_dispatcher.\n"
